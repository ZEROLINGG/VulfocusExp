...
def _double_quotes(cmd: str) -> str:
    ...
    for ch in cmd:
        if ch not in special:
            ...
            result.append(ch)
    # Remove trailing "" if added to last char
    joined = ''.join(result)
    # Clean up: only insert between chars, not at end of tokens
    # Re-do: insert "" after each alnum char except if next is special/space
    result2 = []
    chars = list(cmd)
    for i, ch in enumerate(chars):
        result2.append(ch)
        if ch not in special:
            # insert "" unless this is the last char or next is special/end
...
def apply_obf(name: str, cmd: str, **kwargs) -> str | None:
    ...
    if no and not _cmd_check(cmd, no):
        debug_log(f"cmd contains disallowed pattern. name:{name} no:{no} kwargs:{kwargs} cmd:{cmd}","apply_obf")
        return None
    func = entry["func"]
...
def random_obf(cmd: str, obf: list[str] | None = None, depth: int = 4, args: dict[str, str | list[str]] | None = None) -> str:
    ...
    current_cmd = cmd
    obf = obf or []
    methods = list(OBFUSCATIONS.keys()) if not obf else list(set(obf) & OBFUSCATIONS.keys())
